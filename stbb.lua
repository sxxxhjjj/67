...
-- ====================== 本地化加载 WindUI（核心修改） ======================
local function LoadWindUILocal()
    -- 尝试从本地文件读取
    ...
    if isfile and isfile(localPath) then
        ...
        if fn then
            ...
            if success and result then
                print("[DYHUB] WindUI loaded from local file: " .. localPath)
                return result
            else
                warn("[DYHUB] Failed to execute local WindUI: " .. tostring(result))
            end
        else
            warn("[DYHUB] Failed to loadstring local WindUI")
        end
    else
        print("[DYHUB] Local WindUI not found, will fallback to network.")
    end
    ...
    if success and result then
        print("[DYHUB] WindUI loaded from network: " .. url)
        return result
    else
        error("WindUI loading failed from both local and network: " .. tostring(result))
    end
end

local WindUI = LoadWindUILocal()
if not WindUI then error("WindUI loading failed") end

-- ====================== 以下代码与原 stbb(1).lua 完全一致 ======================
